...
import { useCreateSale, useCompleteSale } from "@/hooks/useSale";
...
export default function POSPage() {
  ...
  // Core Master Catalog Queries & Mutations
  const { data: storesData } = useStores({ size: 1000 });
  const { data: productsData, isLoading: isCatalogLoading } = useProducts({ size: 1000 });
  ...
  // Set up default store selection once loaded
  useEffect(() => {
  ...
  // Dynamic Lucide icons based on category names
  const getCategoryIcon = (catName: string) => {
    const name = catName.toLowerCase();
    if (name === "all menu") return <ShoppingBagIcon className="size-3.5" />;
    if (name.includes("food") || name.includes("drink") || name.includes("beverage")) return <CoffeeIcon className="size-3.5" />;
    if (name.includes("electron") || name.includes("tech") || name.includes("phone")) return <TvIcon className="size-3.5" />;
    if (name.includes("book") || name.includes("stationery")) return <BookOpenIcon className="size-3.5" />;
    if (name.includes("cloth") || name.includes("apparel") || name.includes("wear")) return <LayersIcon className="size-3.5" />;
    if (name.includes("service") || name.includes("labor")) return <ScissorsIcon className="size-3.5" />;
  ...
  // Fallback high-resolution food placeholders matching visual menu mockup
  ...
  const handleSubmitAction = (checkoutType: "HOLD" | "CHECKOUT") => {
    ...
    createSaleMut.mutate(invoicePayload, {
      onSuccess: (res) => {
        const sale = res?.payload;
        if (checkoutType === "CHECKOUT" && sale && sale.id) {
          completeSaleMut.mutate(sale.id, {
            onSuccess: () => {
              toast.success(`Invoice ${sale.invoiceNo} checked out and paid!`);
              setReceiptOpen(true);
            }
          });
        } else {
          toast.success(`Invoice ${sale?.invoiceNo || "Order"} placed on Hold.`);
          handleClearCart();
        }
      },
      onError: () => {
  ...
  const isMutLoading = createSaleMut.isPending || completeSaleMut.isPending;

  return (
    <div className="flex flex-col gap-0 h-[calc(100vh-var(--header-height)-calc(var(--spacing)*12))] overflow-hidden bg-background select-none rounded-xl border border-border/80">
      
      {/* ── POS SubHeader status indicators ── */}
      <div className="bg-muted/30 border-b border-border/60 px-5 py-3 flex flex-row items-center justify-between gap-4 shrink-0 h-14">
        <div className="flex items-center gap-2">
          <CircleDollarSignIcon className="size-4.5 text-primary" />
          <h2 className="text-sm font-extrabold tracking-tight text-foreground uppercase">POS Cashier Till</h2>
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse ml-1" />
          <span className="text-[10px] text-muted-foreground font-semibold uppercase">Register Active</span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={customerId} onValueChange={setCustomerId} disabled={isMutLoading}>
              <SelectTrigger id="pos-page-cust" className="w-[150px] h-8 text-xs font-semibold shadow-sm bg-background">
          ...
          {/* Outlet Selection */}
          <div className="flex items-center gap-2">
            <Label htmlFor="pos-page-store" className="text-[10px] font-black uppercase text-muted-foreground shrink-0">Outlet:</Label>
            <Select value={storeId} onValueChange={setStoreId} disabled={isMutLoading}>
              <SelectTrigger id="pos-page-store" className="w-[160px] h-8 text-xs font-semibold shadow-sm bg-background">
      ...
      {/* ── Main Two-Column register Workspace ── */}
      <div className="flex-1 min-h-0 w-full grid grid-cols-12 overflow-hidden">
        
        {/* ── Left Column: Reactive Menu Grid ── */}
        <div className="col-span-8 flex flex-col min-h-0 bg-slate-50/50 dark:bg-slate-950/20 p-5 overflow-hidden border-r border-border/50">
          
          {/* Search menu and filter bar */}
          <div className="flex gap-3 items-center shrink-0 mb-4.5">
            <div className="relative flex-1">
              <SearchIcon className="absolute left-4 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <Input
                placeholder="Search menu here..."
                className="pl-10 h-10.5 rounded-xl shadow-sm bg-background border-border/70 text-sm"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                disabled={isCatalogLoading}
              />
            </div>
            <Button variant="outline" className="h-10.5 rounded-xl gap-2 font-semibold text-xs border-border/85 bg-background text-muted-foreground hover:text-foreground">
            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`px-4.5 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all duration-200 border cursor-pointer flex items-center gap-1.5 ${
                    isSelected
                ...
                {filteredProducts.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Info Panel */}
                      <div className="flex flex-col items-center text-center mt-2.5 flex-1">
                        <h4 className="text-xs font-bold text-foreground line-clamp-1 leading-snug px-1">
                          {p.name}
                        </h4>
                        ...
                        {hasSelected ? (
                          <div className="flex items-center justify-between bg-slate-100 dark:bg-slate-900 rounded-full p-0.5 border border-border/60 transition-all duration-200">
                            <Button
                              ...
                              variant="ghost"
                              size="icon"
                              className="size-7.5 rounded-full bg-white dark:bg-background shadow text-muted-foreground hover:text-foreground shrink-0 cursor-pointer"
                            ...
                            <Button
                              ...
                              variant="ghost"
                              size="icon"
                              className="size-7.5 rounded-full bg-primary text-primary-foreground shadow hover:bg-primary/90 shrink-0 cursor-pointer"
                              onClick={() => handleUpdateProductQty(p, 1)}
                              disabled={isMutLoading}
                            >
                          ...
                          <button
                            ...
                            disabled={isMutLoading}
                          >
                            <div className="size-7.5 rounded-full bg-white dark:bg-background shadow flex items-center justify-center text-muted-foreground/30 shrink-0">
                              <MinusIcon className="size-3" />
                            </div>
                            <span className="text-xs font-bold text-muted-foreground font-mono">0</span>
                            <div className="size-7.5 rounded-full bg-primary text-primary-foreground shadow flex items-center justify-center shrink-0">
                              <PlusIcon className="size-3" />
                            </div>
                          </button>
                        )}
        ...
        {/* ── Right Column: Staged Cart Panel ── */}
        <div className="col-span-4 flex flex-col min-h-0 bg-background overflow-hidden">
          ...
          <div className="px-5 py-4 border-b border-border/50 bg-slate-50/30 dark:bg-slate-950/5 flex flex-col gap-2 shrink-0">
            <div className="flex items-center justify-between">
              <span className="text-sm font-extrabold text-foreground tracking-tight">Current Orders</span>
              <span className="font-mono text-xs font-black text-foreground">#569124</span>
            </div>
              ...
              {["Dine In", "To Go", "Delivery"].map((type) => {
                ...
                return (
                  <button
                    ...
                    className={`py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all cursor-pointer ${
                      isSelected
            ...
            {cartItems.length === 0 ? (
              ...
              cartItems.map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-2 rounded-2xl border border-border/70 bg-slate-50/50 dark:bg-slate-950/20 relative group hover:border-border transition-all duration-150 shrink-0"
                  >
                    <div className="flex items-center gap-2.5 pr-8 min-w-0">
                      <div className="size-10 rounded-lg overflow-hidden shrink-0 border border-border/50">
                        <img
                    ...
                    <button
                      type="button"
                      className="absolute right-2 opacity-0 group-hover:opacity-100 p-1 text-muted-foreground hover:text-rose-500 rounded transition-all duration-150 shrink-0 cursor-pointer"
          ...
          {/* Subtotals inputs and checkout triggers */}
          <div className="shrink-0 p-5 border-t border-border/60 bg-slate-50/30 dark:bg-slate-950/5 flex flex-col gap-4">
            
            {/* Order notes */}
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="pos-page-note" className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Checkout Notes</Label>
              <Textarea
                id="pos-page-note"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                <Label htmlFor="pos-page-disc" className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Discount ($)</Label>
                <Input
                  id="pos-page-disc"
                  type="number"
                  step="0.01"
                  min="0"
                  placeholder="0.00"
                  className="h-8 text-xs font-mono border-border/70 bg-background"
                  value={discountAmount || ""}
                  onChange={(e) => setDiscountAmount(e.target.value === "" ? 0 : Math.max(0, Number(e.target.value)))}
                  disabled={isMutLoading || cartItems.length === 0}
                />
              </div>
              <div className="flex flex-col gap-1">
                <Label htmlFor="pos-page-tax" className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Tax ($)</Label>
                <Input
                  id="pos-page-tax"
                  type="number"
                  step="0.01"
            ...
            <div className="grid grid-cols-12 gap-2.5 mt-1 shrink-0">
              ...
              <Button
                ...
                disabled={isMutLoading || !storeId || cartItems.length === 0}
              >
                {isMutLoading ? (
      ...
      <Dialog open={receiptOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-[420px] rounded-2xl border border-border bg-background p-6 shadow-2xl flex flex-col gap-0 select-none">
          <div className="flex flex-col items-center text-center pb-4 border-b border-dashed border-border/85 shrink-0">
